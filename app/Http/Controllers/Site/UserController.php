...
class UserController extends Controller
{
    ...
    public function validateCpf($target, $evaluation_id = null) {
        if ($target == 'billet') {
            $data_page = [
                'title' => '- Boleto do mês',
        ...
        } elseif ($target == 'new_evaluation') {
            $data_page = [
                'title' => '- Login',
                'title_page' => 'Login',
                'instruction' => 'Digite o número de seu CPF',
                'button' => 'Avançar',
            ];
        } else { }

        return view('Site.users.simple_user_validation.validate_cpf', compact('data_page', 'target', 'evaluation_id'));
    }
    ...
    public function saveTokenUserSession(Request $req) {
        ...
        } elseif ($data['target'] == 'new_evaluation') {

            return redirect()->route('site.completed_evaluations.new', $data['evaluation_id']);
        } else { }
