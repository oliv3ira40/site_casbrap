    ...
    <main>
        ...
        <div class="container margin_60_60">
			<div class="main_title">
                ...
                <h6>{{ $data['evaluation']->description }}</h2>
			</div>
			<div class="row">
				<div class="ml-auto">
					<div class="box_general">
                        ...
                        {!! Form::open(['url'=>route('site.completed_evaluations.save')]) !!}
                            ...
                            @foreach ($data['no_topic_question'] as $no_topic_question)
                                ...
                                @if ($no_topic_question->confirmation_text != null)
                                    <div class="m-t-10">
                                        <div class="reviews-container">
                                            <div class="review-box clearfix pl-0 m-b-10">
                                                <div class="rev-content rev_content-{{ $no_topic_question->id }} {{ ($errors->has('quest_w_conf_t_required_reading.'.$no_topic_question->id)) ? 'border-warning' : '' }}" style="padding: 10px 10px 10px 10px;">
                                                    <div class="rev-info font-18">
                                                        <div class="row">
                                                            <div class="col-md-6">
                                                                @if ($no_topic_question->title_confirmation_text != null)
                                                                    <span class="float_left">
                                                                        {{ $no_topic_question->title_confirmation_text }}
                                                                    </span>
                                                                @endif
                                                            </div>
                                                            @if ($no_topic_question->reading_the_mandatory_confirmation_text)
                                                                @if (!Session::has('errors'))
                                                                    <div id="required_reading-{{ $no_topic_question->id }}" class="col-md-6">
                                                                        <span id="warning_reading_required-{{ $no_topic_question->id }}" class="float-right text-warning">
                                                                            <i class="icon-info-circled-alt" style="font-size: 24px;"></i>
                                                                            <small>
                                                                                LEITURA OBRIGATÓRIA
                                                                            </small>
                                                                ...
                                                                @else
                                                                    @if ($errors->has('quest_w_conf_t_required_reading.'.$no_topic_question->id))
                                                                        <div id="required_reading-{{ $no_topic_question->id }}" class="col-md-6">
                                                                            <span id="warning_reading_required-{{ $no_topic_question->id }}" class="float-right text-warning">
                                                                                <i class="icon-info-circled-alt" style="font-size: 24px;"></i>
                                                                                <small>
                                                    ...
                                                    <div class="rev-text content_confirmation_text-{{ $no_topic_question->id }}" style="height: 300px; overflow-y: scroll; text-align: justify; padding-right: 20px;">
                                                        {!! nl2br($no_topic_question->confirmation_text) !!}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                @if ($question_type->tag == 'select')
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group m-b-20">
                                                <label class="mb-0" for="question_{{ $no_topic_question->id }}">
                                                    <p style="text-align: justify; font-weight: 500;" class="mb-0">
                                                ...
                                                @endphp 
                                                {!! Form::select('questions['.$no_topic_question->id.']', $option_selec, null, ['class'=>'form-control', 'id'=>'question_'.$no_topic_question->id]) !!}

                                                @if ($errors->has('questions.'.$no_topic_question->id))
                                                    <small class="pl-0 font-bold text-danger txt-trans-initial font-12">
                                                        {{ $errors->first('questions.'.$no_topic_question->id) }}
                                                    </small>
                                                @endif
                                            </div>
                                        </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group m-b-20">
                                                ...
                                                {!! Form::hidden('questions['.$no_topic_question->id.']', null) !!}
                                                {{-- {!! Form::radio('questions['.$no_topic_question->id.']', 0, true, ['style'=>'display:none;']) !!} --}}
                                                @foreach ($possibles_question_answers as $answer)
                                                    <span class="p-r-10 span_question">
                                                        {!! Form::radio('questions['.$no_topic_question->id.']', $answer->id, false, ['class'=>'form-control', 'id'=>$answer->id, 'style'=>'width: auto; display: unset; min-height: auto; height: auto;']) !!}
                                                        <label for="{{ $answer->id }}" class="mb-0">
                                                            {{ $answer->response_text }}
                                                        </label>
                                                    </span>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group m-b-20">
                                                ...
                                                {!! Form::textarea('textarea_questions['.$no_topic_question->id.']', null, ['class'=>'form-control', 'id'=>'question_'.$no_topic_question->id, 'rows'=>'5', 'style'=>'height:100px;']) !!}
                                                @if ($errors->has('textarea_questions.'.$no_topic_question->id))
                                                    <small class="pl-0 font-bold text-danger txt-trans-initial font-12">
                                                        {{ $errors->first('textarea_questions.'.$no_topic_question->id) }}
                                                    </small>
                                                @endif
                                            </div>
                                        </div>
